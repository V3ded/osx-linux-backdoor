...
~
It creates a hidden user-supplied directory inside victim's home root. A check is made if the current system has netcat installed and chooses a payload appropriately. Afterwards, a cronjob gets created which calls the script every minute by default. Simply use **nc -lvp PORT** to catch your shell. 
